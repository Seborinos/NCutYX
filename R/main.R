...
ncut <- function(Y,
  ...
  Wyy <- similarity_matrix(Y, dist, sigma)
...
#' @param scale boolean defining whether to scale the data.
...
ancut <- function(Y,
  ...
  p <- dim(Y)[2]
  Wyy <- similarity_matrix(Y, dist  = 'correlation', sigma = 0.1)
  ...
  Y2 <- stats::predict(m1, newx=X)
  Y2 <- scale(Y2[ , , 1])
  Wxx <- similarity_matrix(Y2, dist  = 'correlation', sigma = 0.1)
  ...
  for (k in 1:B){
    ...
    if (J2>J){
      des <- stats::rbinom(1,1,exp(-L*log(k+1)*(J2-J)))
      if (des==1){
...
pwncut <- function(X,
  ...
  p <- dim(X)[2]
  Wx <- similarity_matrix(X, dist, sigma, epsilon)
  ...
  for (j in 1:nstarts){
    ...
    if(is.matrix(start)==T){
      ...
      Cx=matrix(stats::runif(p*K),p,K)
      Sums=apply(Cx,1,sum)
      Cx=Cx/Sums
    } else if(start=='kmeans'){
      results<-stats::kmeans(t(X),centers=K)
      Cx=matrix(0,p,K)
      for (i in 1:p){
        Cx[i,results[[1]][i]]=1
      }
    ...
    for (k in 1:B){
      ...
      p_minus=stats::runif(1,min=0,max=C2x[s,cluster])
...
mlbncut <- function(Z,
  ...
  for (j in 1:B){
    ...
    #Calculate \hat{q}
    cutoff       <- as.numeric(stats::quantile(loss,q0,na.rm=T,type=4))
    quantiles[j] <- cutoff
  ...
  for(para in 1:nrow(Para)){
    ...
    while((b<=B)||(sum(ws-ws.old)/sum(ws.old)>=10e-4)){
      ...
      if(OP.value<=OP.value.old){
        des <- stats::rbinom(1, 1, Prob(OP.value, OP.value.old, L, b))
        if(des==1){
          Cs.old <- Cs
          ws.old <- ws
        }else{
